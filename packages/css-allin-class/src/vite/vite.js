...
export default function vitePlugin(userConfig, pluginShare) {
  ...
  return {
    ...
    async transform(source, id) {
      ...
      // 给App.vue添加css占位
      if (
        notAddCssPlaceholder &&
        id.includes('/App.vue?vue&type=style') &&
        (id.endsWith('lang.css') || id.endsWith('lang.scss'))
      ) {
        let ms = new MagicString(source)
    ...
    async generateBundle(options, bundle) {
      ...
      for (const file of files) {
        ...
        if (code.includes(CSS_PLACE_HOLDER_KEY)) {
          const outCssStr = genOutputStr({
            outputCss: pluginShare.outputCssCache
          })
