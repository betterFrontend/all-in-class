...
class ConfigLoader {
  ...
  async loadConfig() {
    ...
    let plugin =
      vueConfigFile?.configureWebpack?.plugins.find(
        plugin => plugin.namename === '111111'
      ) || {}
  ...
  replacNodeModulesPath(configFile, replacedStr, filePath1) {
    ...
    let match = replacedStr.match(
      /(\/\/)*\s+\{\s*(?:(const|var|let))\s*\}\s+(\w+)\s*=\s*require\(\s*['"](.*?)['"]\s*\)/
    )
    if (match) {
  ...
  replacRelativePath(configFile, replacedStr, filePath1) {
    ...
    if (match) {
      ...
      replacedStr = replacedStr.slice(match.index + match[0].length)
      configFile =
        this.replacRelativePath(configFile, replacedStr) || configFile
      return configFile
    }
    return configFile
  }
