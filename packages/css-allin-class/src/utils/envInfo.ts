...
export function initEnv() {
  // 判断是否uniapp
  if (
    process.UNI_OUTPUT_DIR ||
    process.UNI_MANIFEST ||
    ...
    if (
      process.argv[1].endsWith('vite.js') ||
      process.env.VITE_USER_NODE_ENV ||
      process.env.UNI_CLI_CONTEXT.endsWith('uniapp-cli-vite') ||
      process.cwd().endsWith('uniapp-cli-vite')
    ) {
      ...
      process.env.VUE_CLI_SERVICE ||
      process.argv[1].endsWith('vue-cli-service.js') ||
      process.env.UNI_CLI_CONTEXT.endsWith('uniapp-cli') ||
      process.cwd().endsWith('uniapp-cli')
    ) {
      info.packFramework = 'webpack'
      info.projectRoot = process.env.VUE_CLI_SERVICE_CONFIG_PATH?.replace(
        '\\vue.config.js',
        ''
      )
      if (info.isCli) {
